...
import id.jsu.suntiq.utils.listener.RecyclerViewItemClickListener
...
abstract class BaseLoadingAdapter<T>(context: Context) : BaseAdapter<T, BaseHolder<T>>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        when (viewType) {
            LOADING_VIEW ->
                LoadingViewHolder(
                    LayoutInflater.from(parent.context).inflate(layoutLoading(), parent, false),
                    listener
                )
            else ->
    ...
    inner class LoadingViewHolder(itemView: View, listener: RecyclerViewItemClickListener<T>) :
